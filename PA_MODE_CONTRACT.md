...
**Console header:** left = scope title (`All 403 neighbourhoods` / `<Name> · rank · parcels · state` / `N neighbourhoods selected`) + metric chips; right = District ▾ (facet, brush-fenced), × Clear (deselect all — new control, its only action is emptying selection), Export (as-built menu, portal to body preserved).
...
- **Brush fence (VIEW-only, LOCKED):** `brushActive = !selectionMode && columnFilters.length>0` → `brushedIds` → map dim ONLY. Never selection/aggregate/export. District facet + range are the only brush writers; removing the rack must not touch this.
