...
export function makeResetControl(onClick, label = "Reset view") {
  return {
    onAdd() {
      ...
      // "Fit view / recentre" glyph — corner brackets framing a centre dot (fit-to-extent
      // semantic), NOT the old GPS/locate crosshair (this control recentres/fits, it does not
      // geolocate). Matches the control's "Reset view" behaviour + label.
      btn.innerHTML =
        '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" ' +
        'stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" ' +
        'style="display:block;margin:auto"><path d="M4 8V4h4"/><path d="M16 4h4v4"/>' +
        '<path d="M20 16v4h-4"/><path d="M8 20H4v-4"/><circle cx="12" cy="12" r="1.6" fill="currentColor" stroke="none"/></svg>';
